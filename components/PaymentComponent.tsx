...
import { View, StyleSheet, Text, FlatList, Image, ActivityIndicator, Alert, Modal } from 'react-native';
import { Button } from 'react-native-paper';
import { WebView } from 'react-native-webview';
import AsyncStorage from '@react-native-async-storage/async-storage';
import { useCartViewModel } from '../src/viewmodels/CartViewModel';

const PaymentComponent = ({ visible, onClose }: { visible: boolean; onClose: () => void }) => {
  ...
  const handleNavigationStateChange = async (navState: any) => {
    ...
    if (url.includes('success')) {
      ...
      try {
        ...
        await new Promise((resolve) => setTimeout(resolve, 5000)); // Simular tiempo de procesamiento
